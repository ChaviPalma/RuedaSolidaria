...
from modelo.administrador import AdministradorModel  # Asegúrate de que este modelo esté implementado correctamente
...
def crear_administrador():
    if request.method == 'POST':
        ...
        if not admin_ID or not pnombre_admin or not snombre_admin or not email or not contrasena:
...
def actualizar_administrador(admin_ID):
    ...
    if request.method == 'POST':
        ...
        if not pnombre_admin or not snombre_admin or not email:
            flash('Todos los campos son obligatorios.', 'error')
